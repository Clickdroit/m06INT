...
int IRSnifferCirpark::DetecterLesCapteurs(unsigned short adresseCapteurDepart, unsigned short adresseCapteurFin)
{
	...
	for (unsigned short adr=adresseCapteurDepart; adr<adresseCapteurFin; adr++)
	{
		if(DetecterUnCapteur(adr) == true)
		{
			...
			char adresseC[5]; sprintf(adresseC,"%.4X",tabCapteurs[nbCapteurs]);
			tabCapteursHexa[nbCapteurs++]=adresseC;
		}
	}
	return nbCapteurs;
}
...
{   switch (codeF)
	{	case 0x03:
		...
		case 0x42:
			...
			break;
		case 0x07:
			if(reponse[2]==0) mddt="Détection du sol"; else mddt="Pas de détection du sol";
			break;
		case 0x04:
			pote=(int)reponse[2]&0xff;
			break;
		case 0x06:
			if(reponse[2]==0x08) {dist="De 150 à 159 cm";      intDist=155;}
			if(reponse[2]==0x09) {dist="De 160 à 179 cm";      intDist=170;}
			if(reponse[2]==0x0A) {dist="De 180 à 189 cm";      intDist=185;}
			if(reponse[2]==0x0B) {dist="De 190 à 209 cm";      intDist=200;}
			if(reponse[2]==0x0C) {dist="De 210 à 229 cm";      intDist=220;}
			if(reponse[2]==0x0D) {dist="De 230 à 249 cm";      intDist=240;}
			if(reponse[2]==0x0E) {dist="De 250 à 269 cm";      intDist=260;}
			if(reponse[2]==0x0F) {dist="De 270 à 279 cm";      intDist=275;}
			if(reponse[2]==0x10) {dist="De 280 à 299 cm";      intDist=290;}
			if(reponse[2]==0x11) {dist="De 300 à 319 cm";      intDist=310;}
			if(reponse[2]==0x12) {dist="De 320 à 339 cm";      intDist=330;}
			if(reponse[2]==0x13) {dist="De 340 à 359 cm";      intDist=350;}
			if(reponse[2]==0x14) {dist="De 360 à 379 cm";      intDist=370;}
			if(reponse[2]==0x15) {dist="De 380 à 399 cm";      intDist=390;}
